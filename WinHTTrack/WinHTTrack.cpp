...
{
  // Eviter deux fen�tres (un seul document)
  // Le CMultui..->CSingleDoc.. est trop complexe � changer (� cause du splitter-wnd)
  int count=1;
  ...
  // Ouvrir nouveau?
  //if (count)
  //  return;       // ne rien faire, car limit� � 1 document
  //count++;
...
void CWinHTTrackApp::DeleteTabs() {
  if (m_tab0)
  if (m_tab0->GetSafeHwnd())       /* a d�ja �t� d�truit par CWinApp */
    delete m_tab0;
  if (m_tab1)
