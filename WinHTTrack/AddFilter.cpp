...
BOOL CAddFilter::OnInitDialog() 
{
  ...
  if (LANG_T(-1)) {    // Patcher en fran�ais
    ...
    SetDlgItemTextCP(this, IDC_STATIC_rule,LANG(LANG_B6) /*"Choisir une r�gle:"*/);
    SetDlgItemTextCP(this, IDC_affkeyword,LANG(LANG_B7) /*"Entrer un mot cl�:"*/);
...
void CAddFilter::OnSelchangeAFtype() 
{
  ...
  switch(r) {
  case CB_ERR : break;  // pas de s�lection
  case 0:
...
{
  switch(ID) {
    case IDC_AFtype: return LANG(LANG_B1); /*"Select a rule for the filter","Choisissez une r�gle pour le filtre");*/ break;
    case IDC_AFext:  return LANG(LANG_B2); /*"Enter here keywords for the filter","Entrez ici un mot cl� pour le filtre");*/ break;
    case IDCANCEL:   return LANG(LANG_B3); /*"Cancel","Annuler");*/ break;
    case IDOK:       return LANG(LANG_B4); /*"Add this filter","Ajouter cette r�gle");*/ break;
    //case : return ""; break;
  }
  return "";
}
