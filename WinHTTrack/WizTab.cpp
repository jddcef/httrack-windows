...
void CWizTab::AddControlPages()
{
	...
	m_psh.dwFlags |= PSP_USEHICON;  // utiliser ic�ne
	...
	/* uniquement pour calibrer la page, retir� lors de initdialog */
	if (is_inProgress)
...
// L'utilisateur a appuy� sur "Apply"
...
// Sauver et appliquer les pr�f�rences
void CWizTab::ApplyAndSave() {
	CWaitCursor wait;      // Afficher curseur sablier
	bool err = false;  // Erreur lors de l'�criture des param�tres
...
void CWizTab::OnOK( ) {
  // Sauver et appliquer pr�f�rences
  ApplyAndSave();
}
void CWizTab::OnCancel( ) {
  // Recharger pr�f�rences
  LoadPrefs();
}
*/
