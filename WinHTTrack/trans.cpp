  ...
  // Patcher l'interface pour les Fran�ais ;-)
	if (LANG_T(-1)) {    // Patcher en fran�ais
	  //SetDlgItemTextCP(this, ,"");
		SetWindowTextCP(this, LANG(LANG_J9) /*"D�marrer.."*/);
		SetDlgItemTextCP(this, IDC_select_start, LANG(LANG_J10) /*"Vous pouvez d�marrer le miroir en pressant la touche DEMARRER,\nou d�finir avant les options de connexion"*/);
		SetDlgItemTextCP(this, IDC_select_save, LANG(LANG_J10b));
		SetDlgItemTextCP(this, IDC_STATIC_delay, LANG(LANG_J11) /*"Retarder"*/);
		SetDlgItemTextCP(this, IDC_wait, LANG(LANG_J12) /*"Attendre avant de commencer jusqu'�: (hh/mm/ss)"*/);
		...
		SetDlgItemTextCP(this, IDC_cnx, LANG(LANG_J15) /*"Connecter � ce provider"*/);
	...
	if (LibRasUse && fill) {
		...
		if (adr) {
			...
			if (LibRas->RasEnumEntries(NULL, NULL, (RASENTRYNAME*)adr, &size, &ent) == 0) {
				...
				m_ctlrasid.InsertString(-1, LANG(LANG_J2 /*"do not connect to a provider (already connected)","pas de connexion � un provider (d�ja connect�)"*/));
				for (i = 0; i < (int)ent; i++) {
...
char* Ctrans::GetTip(int ID)
{
	...
	case IDC_avant: return LANG(LANG_J5); break; // "Back to the start page","Retour � la premi�re page"); break;
	case IDOK:      return LANG(LANG_J6); break; // "Click to start!","Clic pour d�marrer!"); break;
...
BOOL Ctrans::OnWizardFinish()
{
	...
	int r = m_ctlrasid.GetCurSel();
	strcpybuff(RasString, "");
	if ((r != CB_ERR) && (r != 0)) {    // s�lection provider
		if (m_ctlrasid.GetLBText(r, RasString) != CB_ERR) {
			if (strlen(RasString) > 0) {
				//
#if USE_RAS
				if (LibRasUse) {
					if (strlen(RasString) > 0) {    // s�lection provider
						BOOL ifpass;
						...
						if (!LibRas->RasGetEntryDialParams(NULL, &dial, &ifpass)) {
							if (!ifpass) {    // pas de pass
								AfxMessageBox(LANG(LANG_J7 /*"No saved password for this connection!","Pas de mot de passe sauv� pour cette connexion!"*/));
								return 0;    // cancel
							}
						} else {
							AfxMessageBox(LANG(LANG_J8 /*"Can not get RAS setup","Impossible d'obtenir les param�tres de connexion"*/));
							return 0;    // cancel
						}
					}
				}
