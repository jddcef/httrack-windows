...
//int INFILLMEM_LOCKED=0;     // refresh m�moire en cours
...
// arr�t: principal demande l'arr�t (termine_requested)
//        thread1 active termine et que thread2 ait fini de refresher
//        thread2 se termine
//        thread1 retourne 0 � hts_loop
//        le robot termine
//        le thread1 active termine, termine le formulaire et se termine
//        principal ayant quitt� le formulaire affiche le message de fin
...
// t existe-t-il comme r�pertoire?
...
int Save_current_profile(int ask) {
	...
	if (ask) {
		...
		int r;
		char msg[256];
		sprintf(msg, "%s?\r\n%s", LANG_SAVEPROJECT, dialog0->GetName().GetString());
...
int check_continue(char* pathlog) {
	...
	if (
		...
		) {  // il existe d�ja un cache pr�c�dent.. renommer
		//if (fexist(fconcat(path_log,"hts-cache/doit.log"))) {    // un cache est pr�sent
		return 1;
		//}
	}
	if (
		...
		) {  // il existe d�ja un ancien cache pr�c�dent.. renommer
		return 1;
	}
...
void compute_options() {
	...
	// ne pas recharger fichiers d�ja pris mais effac�s
	...
	if (maintab->m_option5.m_maxhtml != "" || maintab->m_option5.m_othermax != "") {
		...
		if (maintab->m_option5.m_maxhtml != "") { ShellOptions->maxfile += ","; ShellOptions->maxfile += maintab->m_option5.m_maxhtml; } else { ShellOptions->maxfile += ","; ShellOptions->maxfile += "0"; }
	...
	if (dialog2->m_rasdisc)
		disconnect = 1;     /* d�connexion � la fin */
	else
		disconnect = 0;

	/* autres options: Shutdown */
	if (dialog2->m_rasshut)
		shutdown_pc = 1;     /* �tendre � la fin */
	else
		shutdown_pc = 0;
}
...
// Les routines � d�finir:
int __cdecl httrackengine_check(t_hts_callbackarg* carg, httrackp* opt, const char* adr, const char* fil, int status) {  // appel� par le wizard
	return -1;
}
int __cdecl httrackengine_check_mime(t_hts_callbackarg* carg, httrackp* opt, const char* adr, const char* fil, const char* mime, int status) {  // appel� par le wizard
	ATLTRACE(__FUNCTION__ " : check %s%s : <%s>\r\n", adr, fil, mime);
	return -1;
}
EXECUTION_STATE(WINAPI* SetThreadExecutionState_)(IN EXECUTION_STATE) = NULL;
void __cdecl httrackengine_init(t_hts_callbackarg* carg) {    // appel� lors de l'init de HTTRACK, avant le d�but d'un miroir
...
void __cdecl httrackengine_uninit(t_hts_callbackarg* carg) {  // appel� en fin de miroir (peut �tre utile!!!)
	...
	// Disconnect RAS
#if USE_RAS
	if (LibRasUse) {        /* librairie RAS charg�e */
		if (disconnect) {     /* on doit d�connecter */
			if (connected) {    /* on a initi� une connexion */
				if (conn)
					LibRas->RasHangUp(conn);
			} else {            /* tout d�connecter */
...
int __cdecl httrackengine_start(t_hts_callbackarg* carg, httrackp* opt) {   // appel� lors du d�marrage du miroir (premi�res requ�tes)
	ATLTRACE(__FUNCTION__ " : starting\r\n");
#if USE_RAS
	// connexion RAS
	has_started = 1;    // d�marrage
	connected = 0;
	conn = NULL;
	memset(&SInfo, 0, sizeof(SInfo));
	if (LibRasUse) {
		if (ShellOptions->_RasString.GetLength() > 0) {    // s�lection provider
			if (!LibRas->RasDial(NULL, NULL, &ShellOptions->_dial, NULL, NULL, &conn)) {
				...
				strcpybuff(connected_err, LANG(LANG_F3 /*"Could not connect to provider","Impossible d'�tablir la connexion"*/));
				connected = -1;
				//termine=1;
			}
		}
...
int  httrackengine_end(t_hts_callbackarg* carg, httrackp* opt) {     // appel� lors de la fin du miroir (plus de liens � charger)
...
int __cdecl httrackengine_htmlcheck(t_hts_callbackarg* carg, httrackp* opt, char* html, int len, const char* url_address, const char* url_file) {    // appel� � chaque fois qu'un html doit �tre scann� (utile pour la prospection mais inutile ici)
	return 1;
}
...
// (plus efficace)
// -->C'est elle qui d�cide de tout arr�ter si elle d�tecte in termine_request<--
int __cdecl httrackengine_loop(t_hts_callbackarg* carg, httrackp* opt,
	...
	int stat_time,
	hts_stat_struct* stats) {    // appel� � chaque boucle de HTTrack
	static char s[HTS_URLMAXSIZE * 2] = "";  // utilis� plus loin
	int stat_written = -1;
	int stat_updated = -1;
	int stat_errors = -1;
	int stat_warnings = -1;
	...
	if (((tl - last_time) >= HTS_SLEEP_WIN) || ((tl - last_time) < 0)) {   // chaque 250 ms
		...
		// pour �viter temps cpu consomm� trop grand
		// Sleep(10);
	  ...
	  // calculer heure si ce n'est d�ja fait
		if (stat_time < 0)
		...
		if ((stat_time > 0) && (stat_bytes_recv > 0))
		  ...
		  // stocker infos: octets transf�r�s, temps, etc.
		...
		// back: tableau de back_max �l�ments de cache
		// back_max: nombre d'�l�ments ^^^^
		// lien_tot: nombre total de liens trait�s pour le moment
		// stat_bytes: octets sauvegard�s
		// stat_bytes_recv: octets t�l�charg�s
		// stat_time: temps en seconde depuis le d�but du miroir
		// stat_nsocket: nombre de sockets connect�es actuellement
		// on peut en d�duire rate=stat_bytes_recv/stat_time
						  ...
						  // int ok=0;  // OPTI
							...
							case 0:     // prioritaire
								if ((back[i].status > 0) && (back[i].status < 99)) {
									strcpybuff(StatsBuffer[index].etat, LANG(LANG_F4 /*"receive","r�ception"*/)); ok = 1;
								}
								break;
							case 1:
								if (back[i].status == 99) {
									strcpybuff(StatsBuffer[index].etat, LANG(LANG_F5 /*"request","requ�te"*/)); ok = 1;
								...
								break;
							default:
								if (back[i].status == 0) {  // pr�t
									if ((back[i].r.statuscode == 200)) {
										strcpybuff(StatsBuffer[index].etat, LANG(LANG_F8 /*"ready","pr�t"*/)); ok = 1;
...
#endif
	// INFILLMEM_LOCKED=0;    // d�locker interface
	}
	WHTT_UNLOCK();
	return (termine == 0);
	}
		...
		if (SInfo.refresh) {
				...
				if (!soft_term_requested) {
					if (!hts_setpause(global_opt, -1)) {
						if (!(parsing = hts_is_parsing(global_opt, -1)))
							SetDlgItemTextCP(inprogress, IDC_inforun, LANG(LANG_F10 /*"Receiving files.","R�ception des fichiers"*/));
						else {
			...
			}
			{
				...
				if ((SInfo.stat_nsocket == -1)) {
					...
					if (!has_started)
#endif
						SetDlgItemTextCP(inprogress, IDC_nm0, LANG(LANG_F15 /*"Waiting for specific hour to start","Attente de l'heure programm�e pour d�marrer"*/));
#if USE_RAS
					else
					...
					if (icn && (!this_CSplitterFrame->iconifie)) {  // minimis�e mais pas en icone
				...
				if (strcmp(info, last_info)) {       /* a chang� */
					strcpybuff(last_info, info);           /* recopier */
					if (this_CSplitterFrame->iconifie)  // minimis� icone
...
void lance(void) {
	...
	// mode spider, mettre apr�s options
	if (ShellOptions->choixdeb[0] == '!') {
		args.Add("--testlinks");
	...
	if (!termine) {
		int result = 0;
		{
			...
			// on efface le doit.log, pour annuler les parametres anciens et en red�finir de nouveaux
			// c'est ici une logique qui diff�re de la version en ligne de commande
...
#error "Non support�"
	...
	/* Aborted mirror or finished? */
		...
		if (result) {      // erreur?
			...
			strcpybuff(end_mirror_msg, LANG(LANG_F22 /*"The mirror is finished.\nClick OK to quit WinHTTrack.\nSee log file(s) if necessary to ensure that everything is OK.\n\nThanks for using WinHTTrack!","Le miroir est termin�\nCliquez sur OK pour quitter WinHTTrack\nVoir au besoin les fichiers d'audit pour v�rifier que tout s'est bien pass�\n\nMerci d'utiliser WinHTTrack!"*/));
...
#endif
		{
			...
			if (strlen(pathlog) > 0)
				if ((pathlog[strlen(pathlog) - 1] != '/') && (pathlog[strlen(pathlog) - 1] != '\\'))
					strcatbuff(pathlog, "/");
			// fichier log existe ou on est t�l�command� par un !
...
int MyWriteProfileIntFile(FILE * fp, CString dummy, CString name, int value) {
	if (fp) {
		fprintf(fp, "%s=%d\x0d\x0a", name.GetString(), value);
	}
	return 0;
}
...
int MyWriteProfileStringFile(FILE * fp, CString dummy, CString name, CString value) {
	if (fp) {
		fprintf(fp, "%s=%s\x0d\x0a", name.GetString(), profile_code(value.GetBuffer(0)).GetBuffer(0));
	}
	return 0;
}
//
...
//
// Get_profile et Write_profile eux m�mes
//
// path="" -> �crire dans la base (default)
// path="<tmp>" -> �crire dans le fichier tempo commun
// path="<mem>" -> �crire dans le buffer tempo commun
// path="<null>" -> lire default (ill�gal en �criture)
void Write_profile(CString path, int load_path) {
	CWaitCursor wait;
	...
	//if (dialog3.m_hWnd == NULL) {    // pas initialis�
	if (maintab->m_hWnd == NULL) {    // pas initialis�
	  // checkboxes
	...
	if (!(path.IsEmpty())) {
		if (dialog1->m_hWnd == NULL) {    // pas initialis�
...
void Read_profile(CString path, int load_path) {
	...
	if (path == "<tmp>") {     // fichier temporaire
		if (!tmpf)
			...
			fflush(tmpf);
	} else if (path == "<null>") {     // options par d�faut
		path = "<mem>";
...
void Build_TopIndex(BOOL check_empty) {
	...
	//if (toptemplate_header && toptemplate_body && toptemplate_footer) {
	{
		...
		//if (fpo) {
		{
			// verif_backblue(opt, path);    // g�n�rer gif
			//
			// Header
