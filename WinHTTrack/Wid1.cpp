...
BOOL Wid1::OnInitDialog( ) {
  ...
  // Patcher l'interface pour les Fran�ais ;-)
  if (LANG_T(-1)) {    // Patcher en fran�ais
    ...
    //SetDlgItemTextCP(this, IDC_regdef,LANG(LANG_G37)); // "Sauver pr�fs");
    ...
    //SetDlgItemTextCP(this, IDfilter,LANG(LANG_G39)); // "D�finir..");
    SetDlgItemTextCP(this, ID_setopt,LANG(LANG_G40)); // "D�finir les options..");
  ...
  //OnChangepathlog();  // update r�pertoire log
  //OnChangepthmir();  // update r�pertoire miroir
      ...
      if ((tempo[i]==' ') || (tempo[i]==0)) {
        ...
        if ((strlen(ch)>0) && ((int) strlen(ch)<HTS_URLMAXSIZE) ) {
          ...
          if (ident_url_absolute(ch,adr,fil)==-1) {
            htsblk r;
            char loc[HTS_URLMAXSIZE*2]; loc[0]='\0';    // �ventuelle nouvelle position
...
void Wid1::OnSelchangetodo() 
{
  ...
  if (r!=CB_ERR) {
    ...
    case 0:
      ...
      "Mode miroir, remplissez les addresse(s) dans la liste d'URLs.\x0d\x0aLe mirroir sera sauv� � l'emplacement indiqu� plus bas"*/)
      );
      break;
    case 1:
      SetDlgItemTextCP(this, IDC_INFOMAIN,
      ...
      "Mode miroir semi automatique (pose des questions), remplissez les addresse(s) dans la liste d'URLs.\x0d\x0aLe mirroir sera sauv� � l'emplacement indiqu� plus bas"*/)
      );
      break;
    case 2:
      SetDlgItemTextCP(this, IDC_INFOMAIN,
      ...
      "Mode t�l�chargement de fichier, remplissez les addresse(s) des fichiers dans la liste d'URLs.\x0d\x0aLe mirroir sera sauv� � l'emplacement indiqu� plus bas"*/)
      );
    ...
    case 4:
      ...
      "Mode test de liens, remplissez les adresse(s) des pages contenant les liens � tester dans la liste d'URLs.\x0d\x0aLes fichiers d'audit seront sauv�s � l'emplacement indiqu� plus bas"*/)
      );
      SetDlgItemTextCP(this, IDC_depth,"");
      break;
    default:
      if (r == LAST_ACTION )
        ...
        "Mode mise � jour/continuer un miroir, v�rifiez les adresse(s) dans la liste d'URLs, puis cliquez sur le bouton 'NEXT' et v�rifiez les param�tres.\x0d\x0aLe mirroir sera sauv� � l'emplacement indiqu� plus bas"*/)
        );
...
void Wid1::AfterChangepathlog() 
{
  ...
  {
    if (fexist(fconcat(catbuff,sizeof(catbuff),tempo,"hts-cache/winprofile.ini"))) {    // un cache est pr�sent
      if (fsize(fconcat(catbuff,sizeof(catbuff),tempo,"hts-cache/winprofile.ini"))>0) {   // taille log contr�le>0
        int i;
        for(i=0;i<(int) strlen(tempo);i++)
          if (tempo[i]=='/')
            tempo[i]='\\';
...
char* Wid1::GetTip(int ID)
{
  switch(ID) {
    ...
    //case IDC_urls:  return LANG(LANG_G12); break; // "Check spelling","V�rifier syntaxe"); break;
    //case IDfilter:  return LANG(LANG_G13); break; // "Define additional filters","D�finir les filtres suppl�mentaires"); break;
    ...
    case IDC_regdef:   return LANG(LANG_G22); break; // "Save preferences as default values","Sauver les r�glages par d�faut"); break;
    case IDOK:         return LANG(LANG_G23); break; // "Click to continue","Clic pour continuer"); break;
    case IDC_avant:     return LANG_TIPPREV; break;
    case ID_setopt:    return LANG(LANG_G24); break; // "Click to define option","Clic pour d�finir les options"); break;
...
void Wid1::OnDropFiles(HDROP hDropInfo) 
{
  ...
  for(i=0;i<nf;i++) {
    char* buff;
    int size;
    size=DragQueryFile(hDropInfo,i,NULL,0);    // taille buffer n�cessaire
    if (size>0) {
      size+=16;    // marge de s�curit�
      buff=(char*) calloc(size,1);
      if (buff) {
        if (DragQueryFile(hDropInfo,i,buff,size)>0) {    // ok copi�
...
CString Wid1::TextToUrl(CString st,CLIPFORMAT cfFormat) {
  // yop
  // on va convertir les chaines � la suite
  CString res;
  if (cfFormat==CF_TEXT) {
    char *buff, *last;
...
void Wid1::OnLoadprofile() {
  ...
  strcpybuff(szFilter,LANG(LANG_G25 /*"WinHTTrack preferences (*.opt)|*.opt||","R�glages de WinHTTrack (*.opt)|*.opt||"*/));
